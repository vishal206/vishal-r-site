  ...
  useEffect(() => {
    const fetchDevLogs = async () => {
      try {
        ...
        const fetchedDevLogs = (await Promise.all(devLogPromises))
          .filter(devLog => devLog !== null) as DevLogMeta[];
          
        // Sort by date (newest first)
        fetchedDevLogs.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
            ...
            <div key={devLog.slug} className="pb-2">
              ...
              >
                <div className="flex items-center md:gap-3 gap-1 text-xs md:text-sm font-sans">
                  ...
                  <span className="text-sm md:text-lg group-hover:underline">{devLog.title}</span>
                  <FontAwesomeIcon 
