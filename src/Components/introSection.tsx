...
import { faDownload, faBriefcase, faSchool } from "@fortawesome/free-solid-svg-icons";
...
const IntroSection: React.FC<IntroSectionProps> = ({ onKnowMoreClick }) => {
  ...
  const getIconScale = (index: number) => {
    ...
    if (distance === 0) return 1.30; // 125% for hovered icon
    if (distance === 1) return 1.10; // 115% for adjacent icons
  ...
  const socialIcons = [
    { icon: faLinkedin, action: () => handleExternalLink("https://linkedin.com/in/vishal-r-profile") },
    { icon: faSquareGithub, action: () => handleExternalLink("https://github.com/vishal206") },
  ...
  return (
    <div className={`bg-background-grid-section rounded-lg md:p-6 p-4 h-full flex flex-col justify-between`}>
      <div>
        ...
        <p className="text-xs md:text-md font-sans text-center mb-4 md:mb-8  text-gray-200">
          I’m Vishal R, a BI & Analytics Engineer working at the intersection of AI, data, and web engineering.
I build products that make data easier to understand — from intelligent dashboards to conversational insights.
        </p>
        
        {/* Social Icons */}
        <div 
          className="flex justify-center gap-5 md:mb-6"
        ...
        >
          {socialIcons.map((item, index) => (
            ...
            >
              <FontAwesomeIcon icon={item.icon} className="text-white text-xl transition-all" />
            </button>
          ))}
        </div>
      </div>
