  ...
  useEffect(() => {
    const fetchWeekNotes = async () => {
      try {
        ...
        setWeekNotes(fetchedWeekNotes.sort((a, b) => 
    ...
    <div className="md:space-y-4">
      {weekNotes.length === 0 ? (
        ...
        weekNotes.map((weekNote) => (
          <div key={weekNote.slug} className="pb-3">
            <Link 
            ...
            >
              <div className="flex items-center md:gap-3 gap-1 text-xs md:text-sm font-sans">
                ...
                <span className={`text-text-secondary`}>Week #{weekNote.weeknoteCount}</span>
                <span className="text-gray-400">•</span>
                <span className="text-sm md:text-lg group-hover:underline">{weekNote.title}</span>
                <FontAwesomeIcon 
