...
const generateRSSFeed = async () => {
  ...
  if (fs.existsSync(devLogsDir)) {
    const projects = fs.readdirSync(devLogsDir).filter(item => 
      fs.statSync(path.join(devLogsDir, item)).isDirectory()
    );
    
    for (const project of projects) {
      const projectDir = path.join(devLogsDir, project);
      const devLogFiles = fs.readdirSync(projectDir).filter(file => file.endsWith('.md'));
